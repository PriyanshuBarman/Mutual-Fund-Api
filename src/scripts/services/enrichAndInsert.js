...
export const enrichAndInsert = async (funds) => {
  ...
  const insertTasks = funds.map((fund) =>
    limit(async () => {
      const isin = fund.isinGrowth;

      try {
        const kuvera = await fetchFundData(isin);
        if (!kuvera || !kuvera.code)
          throw new Error("Missing Kuvera fund code");
        ...
        console.error(`❌ Failed: ${isin}`, error.message);
        failedFunds.push({
          scheme_code: fund.schemeCode,
  ...
  // Wait for all to finish (in batches of 5)
